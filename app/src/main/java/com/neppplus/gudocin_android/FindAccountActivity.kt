...
class FindAccountActivity : BaseActivity() {
    ...
    override fun setupEvents() {
            ...
            apiService.getRequestEmail(inputNickname, inputPhone).enqueue(object :
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        Toast.makeText(mContext, "등록된 연락처로 가입한 이메일을 보냈습니다.", Toast.LENGTH_SHORT).show()
                    } else {
        ...
        binding.btnSubmit2.setOnClickListener {
            ...
            apiService.postRequestPassword(inputEmail, inputNickname).enqueue(object :Callback<BasicResponse>{
                override fun onResponse(
                    call: Call<BasicResponse>,
                    response: Response<BasicResponse>
                ) {
                    if (response.isSuccessful) {
                        Toast.makeText(mContext, "임시 비밀번호를 메일로 전송했습니다.", Toast.LENGTH_SHORT).show()
                    } else{
